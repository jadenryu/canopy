...
5. **Formal `weave.Evaluation`** — `canopy-allocator-eval` benchmarks market allocation vs. four baselines (single cheap agent, single premium agent, random, round-robin) on a held-out job set (25 jobs incl. an unseen category, 3 seeds, saboteurs in the fleet). **Headline: the market matches a hand-vetted single agent (0.98 vs 0.99 quality) at a third of premium's cost (+205% quality-per-dollar), and is the only allocator that survives the two things a fixed setup can't — it routed 0% of jobs to saboteurs (round-robin wasted 20% and never stopped) and absorbed top-agent death with zero price disruption.** Robustness experiments + full table in [`documentation/results.md`](documentation/results.md).
